...
(roughly translated from German), which were measured on a Likert scale of 1: "fully disagree", ..., 5: "fully agree". Notice the indicators are not continuous and therefore deviate from the ideal normality assumption (at best, they can be thought of as 'coarse' or 'rough' measures of the actual normally distributed variable). We will ignore this fact out of convenience. 
...
```{r}
cov(df[, c("px06", "px07", "px10")], use = "complete.obs")
```
...
The indicators are all endogenous variables, so when we specify their variances (also optional), we are referring to the unexplained part, i.e., the measurement error. We use labels here as well, `t1` for 'theta 1', etc. Finally, we can add the variance of the latent variable, which is also optional. This variance is the only exogenous variance in the model. 
...
where $k$ is the number of indicators and $r$ is the average correlation between the three [@Urban2014, p. 20]. By covariance algebra (or tracing), we see that the correlation between any two indicators is given by the product of the standardized factor loadings. So, in this case, we have $\lambda_{11}^{*} = 0.664$, $\lambda_{21}^{*} = 0.699$ and $\lambda_{31}^{*} = 0.481$. So, the model-implied correlation between $x_{1}$ and $x_{2}$ is $0.664 \cdot 0.699 = `r round(0.664 * 0.699, 3)`$, between $x_{1}$ and $x_{3}$ it is $0.664 \cdot 0.481 = `r round(0.664 * 0.481, 3)`$ and between $x_{2}$ and $x_{3}$ it is $0.699 \cdot 0.481 = `r round(0.699 * 0.481, 3)`$. The average correlation is $r = (`r round(0.664 * 0.699, 3)` + `r round(0.664 * 0.481, 3)` + `r round(0.699 * 0.481, 3)`) / 3 = `r round(1 / 3 * (0.664 * 0.699 + 0.664 * 0.481 + 0.699 * 0.481), 3)`$. So, we have 
\begin{align}
\text{REL} & = \frac{3 \cdot 0.373}{1 + (3 - 1)0.373} \\ 
 & = `r round((3 * 0.373) / (1 + (3 - 1)* 0.373), 3)`.
\end{align}

There is a function in the so-called `semTools` package for calculating the reliability of a measurement model with `semTools::reliability`. The package `semTools` provides functions to extend the functionality of `lavaan` (see [https://cran.r-project.org/web/packages/semTools/index.html](https://cran.r-project.org/web/packages/semTools/index.html)). We use `alpha` to calculate the Cronbach's Alpha reliability coefficient (with only a slight discrepancy compared to the statistic calculated above by hand):
...
- $x_{4}$: "Should immigrants be forced to assimilate? [recoded]" (`pa09r`) 
- $x_{5}$: "Should we prohibit the influx of asylum-seekers? [recoded]" (`pa19r`).
...
```{r output.lines=10:14, echo=FALSE}
summary(cfa2.fit, standardized = TRUE, fit.measures = TRUE)
```
...
```{r echo=FALSE, output.lines=36:41}
summary(cfa2.fit, standardized = TRUE, fit.measures = TRUE)
```
...
S   <- round(cor(df[, c("px06", "px07", "px10", "pa09r", "pa19r")], use = "complete.obs"), 3)
...
```{r echo=FALSE, output.lines=43:45}
summary(cfa2.fit, standardized = TRUE, fit.measures = TRUE)
```
...
```{r output.lines=22:25, echo=FALSE}
summary(cfa2.fit, standardized = TRUE, fit.measures = TRUE)
```
