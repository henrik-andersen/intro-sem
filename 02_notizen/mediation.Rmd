...
We use `[,]`, or indexing, to refer to specific rows and columns of this table. For example, the effect `gamma2` is found in second column and the estimate is in column 14:
...
In our mediation model, we can bring the means in, as well. We usually avoid the notation $\beta_{0}$ because we want to save the subscript in order to reference the dependent variable, so we can use $\tau_{1}, \tau_{2}$, etc. 
\begin{align}
